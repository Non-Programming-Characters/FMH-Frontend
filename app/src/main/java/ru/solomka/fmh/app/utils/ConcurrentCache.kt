...
abstract class ConcurrentCache<T : Entity> : Cache<UUID, T> {
    ...
    override fun put(value: T): T = concurrentCache.put(value.id, value)!!

    override fun putAll(value: Array<T>): Boolean {
        value.forEach {
            concurrentCache[it.id] = it
        }
        return true
    }
    ...
    override fun removeByKeyAndObject(value: T): Boolean = concurrentCache.remove(value.id, value)
